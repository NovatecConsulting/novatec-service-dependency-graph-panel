...
class NodeTree {
    ...
    getNodesFromLayer(layer: number) {
        ...
        nodes.forEach((element => {
            if(this._metricMap[element.data.id]) {
                (Object.keys(element.data.metrics) as Array<keyof typeof element.data.metrics>).forEach(key => element.data.metrics[key] = element.data.metrics[key] / this._metricMap[element.data.id][key])
            }
        }
        ))
    ...
    private _getNodesFromLayer(currentNode: NodeTreeElement, layer: number, layerCounter: number): IntGraphNode[] {
        if(layer === layerCounter) {
            var children = currentNode.children.map(element => element.node)
        ...
        layerCounter++;
        var children = _.flatten(currentNode.children.map(element => this._getNodesFromLayer(element,layer, layerCounter)))
        if(currentNode != this._root) {
            children.push(currentNode.node);
        }
    ...
    private _sumMetrics(sourceNode: IntGraphNode, targetNode: IntGraphNode): IntGraphMetrics {
        ...
        if(target.rate || source.rate) {
            ...
            if(target.rate && source.rate && target.rate != NaN && source.rate != NaN) {
                this._metricMap[targetNode.data.id].rate ? this._metricMap[targetNode.data.id].rate = this._metricMap[targetNode.data.id].rate + 1 : this._metricMap[targetNode.data.id].rate = 1
            } else {
        ...
        if(target.response_time || source.response_time) {
            metrics.response_time = (target.response_time ? target.response_time:0 ) +  (source.response_time ? source.response_time:0 )
            if(target.response_time && source.response_time && target.response_time != NaN && source.response_time != NaN) {
                this._metricMap[targetNode.data.id].response_time ? this._metricMap[targetNode.data.id].response_time = this._metricMap[targetNode.data.id].response_time + 1 : this._metricMap[targetNode.data.id].response_time = 1
            } else {
        ...
        if(target.success_rate || source.success_rate) {
            metrics.success_rate = (target.success_rate ? target.success_rate:0 ) +  (source.success_rate ? source.success_rate:0 )
            if(target.success_rate && source.success_rate && target.success_rate != NaN && source.success_rate != NaN) {
                this._metricMap[targetNode.data.id].success_rate ? this._metricMap[targetNode.data.id].success_rate = this._metricMap[targetNode.data.id].success_rate + 1 : this._metricMap[targetNode.data.id].success_rate = 1
            } else {
    ...
    private _getObjectFromArray(array: NodeTreeElement[], id: string) {
        for (var i = 0; i < array.length; i++) {
            if (array[i].node.data.label === id) return array[i]
        }
        return undefined
    ...
    private _addNode(nodeToAdd: NodeTreeElement, currentLayerNode: NodeTreeElement, currentLevel: number) {
        ...
        if(namespaceLength === currentLevel) {
            const possibleDuplicate =  _.find(currentLayerNode.children, function(o) { return o.id === nodeToAdd.id; });
            ...
            const nextLayerNode = this._getObjectFromArray(currentLayerNode.children, nodeToAdd.node.data.namespace[currentLevel])
            if (nextLayerNode === undefined) {
                ...
                const nextTopLayerNode = this._getObjectFromArray(currentLayerNode.children, nodeToAdd.node.data.namespace[currentLevel]);
